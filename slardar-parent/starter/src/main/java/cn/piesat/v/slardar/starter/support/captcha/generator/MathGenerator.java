package cn.piesat.v.slardar.starter.support.captcha.generator;
...
public class MathGenerator implements CodeGenerator {
	...
	private String padAfter(CharSequence str, int minLength, char padChar) {
		if (null == str) {
			return null;
		}
	...
	public boolean verify(String code, String userInputCode) {
		...
		final int a = Integer.parseInt(sub(code, 0, this.numberLength).trim());
		final char operator = code.charAt(this.numberLength);
		final int b = Integer.parseInt(sub(code, this.numberLength + 1, this.numberLength + 1 + this.numberLength).trim());
