...
import cn.piesat.nj.slardar.starter.support.SignatureUtil;
...
import static cn.piesat.nj.slardar.starter.support.HttpServletUtil.sendJson;
...
public class SlardarApiSignatureFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        // TODO: 验证 appKey
        ApiKeyFilterConfig.Client client = config.getByAppKey(appKey);
        if (client == null) {
            exception = new SlardarException("appKey 不存在!");
        }
        // 验证将要访问的接口path 是否被允许
        if (StringUtils.hasText(client.getIncludePattern())) {
            boolean matched = Arrays.stream(client.getIncludePattern().split(","))
        ...
        // 计算签名
        String signature = SignatureUtil.generateSignature(path, appKey, client.getSecretKey(), nonce, requestTime);
        if (signature == null) {
