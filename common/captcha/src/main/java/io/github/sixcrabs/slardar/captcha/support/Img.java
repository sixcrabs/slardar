...
public class Img implements Serializable {
    ...
    private final float diff = 1e-6f;
    ...
    public Img scale(int width, int height, Color fixedColor) {
        ...
        if (Math.abs(widthRatio - heightRatio) < diff) {
    ...
    private static Rectangle calcRotatedSize(int width, int height, int degree) {
        ...
        double angelDeltaWidth = Math.atan((double) height / width);
        double angelDeltaHeight = Math.atan((double) width / height);
